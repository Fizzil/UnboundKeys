...
- **Game mode** — a switch in the editor (it stays on in every editor) that shows the timing a game with a global cooldown needs. **Gap between keys** is how long an infinite repeat waits after each key: set it to your global cooldown, 1.5 s for most WoW classes (a little less with haste), 1.0 s for Rogues, cat-form Druids and Monks; the one-click presets set exactly that. **Priority** marks a key that may interrupt: press it and the infinite repeat pauses, the app waits for the current global cooldown to finish, your key fires, and the repeat stays paused for the time you set (**Pause the repeat for**: Reset pauses the infinite repeat for one GCD; change the pause time for a channelled ability, usually two to three seconds). Two priority keys in a row extend the pause. Any mapping can be a priority key, a timed repeat pauses the same way, and the settings work even with Game mode switched off.
