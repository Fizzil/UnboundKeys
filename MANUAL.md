...
- **Game mode** — a switch in the editor (it stays on in every editor) that shows three more switches for games with a global cooldown. **Global cooldown** belongs to the sub-profile, so to the class: switch it on, pick "Most classes · 1.5 s" or "Rogue, cat Druid, Monk · 1.0 s" (or step the number down a little if haste makes abilities queue up), and every infinite repeat in that sub-profile waits one GCD between keys; off means none is applied. **Custom gaps between keys** makes one mapping wait its own time instead: one gap for all its keys, and a different one after any key if you want. **Priority** marks a key that may interrupt: press it and the infinite repeat pauses, the app waits for the current GCD to finish, your key fires, the repeat pauses one more GCD, then it resumes where it left off. Tick **Channelled ability** on a priority key whose ability channels and set how long the channel takes, usually two to three seconds; the repeat stays paused that long instead of one GCD. Two priority keys in a row extend the pause. Any mapping can be a priority key, a timed repeat pauses the same way, and the settings work even with Game mode switched off.
