...
- **Game mode** — a switch in the editor (it stays on in every editor) that shows the timing a game with a global cooldown needs. **Gap between keys** is how long Repeat waits after each key: match the global cooldown, 1.5 s for most WoW classes (a little less with haste), 1.0 s for Rogues, cat-form Druids and Monks; the one-click presets set exactly that. **Priority** marks a mapping that may interrupt: pressing it pauses any running repeat, waits out the rest of that repeat gap so the game cooldown is over, fires, then holds the repeat for the time you set before it carries on. Reset means one gap, right for an instant ability; a channelled ability wants two or three seconds. A second priority press during a pause extends it.
